 ...
 * @param {OptionStylesObject} props.optionStraightStyles An object containing directed styles for each element. Place the styles directly here without declaring a whole new Styles Object. _Remove props changing the same styles this changes_.
 */
//prettier-ignore
function CuteSelect({
  label = DEFAULT_LABEL,
  ...
  const styles = getActiveStyles({ hover, ...state });
    ...
    <div className={styles.ct}>
      ...
      <div className={styles.listCt}>
        {Object.keys(options).map((opKey) => {
          ...
          return (
            <Option
              customDirSty={optionStraightStyles}
 ...
 * @property {string} ct
 * @property {string} displayCt 
 * @property {string} displayLabel 
 * @property {string} displaySelectionText 
 * @property {string} displayIcon 
 * @property {string} listCt 
 */
const DIRECTED_STYLES = {
  ...
  displaySelectionText: "text-light text-base text-gray-700 text-opacity-0 || oho,ose<text-opacity-100>",
...
const OPTION_DIRECTED_STYLES = {
  ...
  text: "text-sm text-gray-500 text-light transition-all duration-200 || ho,se<text-gray-800>",
