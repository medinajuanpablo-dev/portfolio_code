 ...
 * @param {"outline-and-filled" | "always-outline" | "always-filled"} props.colorsBehavior Tells how the colors should behave on hover and focus.
...
function CuteButton({
  ...
  //prettier-ignore
  const getActiveStyles = useIndicatedStyles(INDICATORS, dirSty, { customDirSty: straightStyles });

  //prettier-ignore
  const styles = getActiveStyles({ size, iconSide, color, iconOnly, transitionSpeed, colorsBehavior });
...
const INDICATORS = [
  ...
  { key: "always-outline", directive: "ao", condition: (p) => p.colorsBehavior == "always-outline" },
  { key: "always-filled", directive: "af", condition: (p) => p.colorsBehavior == "always-filled" },
];
